    ...
    $scope.uploadAvatar = function(files, user_id){
  	if (files && files.length) {
        for (var i = 0; i < files.length; i++) {
            var file = files[i];
            Upload.upload({
		        url: server_host + '/users/' + user_id + '.json',
		        method: 'PUT',
	...
	getPlaylist = function(){
		...
		for (var counter = 0; counter < $scope.songs.length; counter ++){
			songs.push({title: $scope.songs[counter].title,
			            ...
			            url: server_host + '/songs/' + $scope.songs[counter].id + '.json'});
		}
		return songs;
	};
...
function($scope, Auth, audio, Upload){
	...
	for (var counter = 0; counter < $scope.songs.length; counter ++){
		songs.push({title: $scope.songs[counter].song.title,
		            ...
		            url: server_host + '/songs/' + $scope.songs[counter].song.id + '.json'});
	}
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            Upload.upload({
            	...
            	tempPlaylistItems.unshift({title: data.song.title, 
				            			   ...
				            			   likes: 0,
            						       id: data.song.id,
            						       url: server_host + '/songs/' + data.song.id + '.json'});
