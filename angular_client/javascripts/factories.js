...
function($http){
  ...
  o.getUser = function(id){
  	return $http.get( server_host + '/users/' + id + '.json').success(function(res){
      angular.copy(res, o.user);
    });
  };
  
  o.updateUser = function(user){
  	return $http.post(server_host + "/users/" + user.id + '/edit.json', user);
  };
  
  o.getUserpageMedia = function(id){
  	return $http.get( server_host + '/users/' + id + '/userpage_media.json').success(function(res){
      angular.copy(res, o.userpage_media);
    });
  };
  
  o.getUsersFriends = function(id){
  	return $http.get( server_host + "/users/" + id + "/friends.json").success(function(res){
  ...
  o.unsubscribe = function(user_id, friend, callback) {
	  return $http.delete(server_host + '/users/' + user_id + '/subscribtions/' + friend.user.id + '.json').success(function(data){
  ...
  o.addFriend = function(user_id, subscriber, callback) {
	  return $http.post(server_host + '/users/' + user_id + '/subscribtions.json', {subscriber_id: subscriber.user.id}).success(function(data){
  ...
  o.markReqViewed = function(user_id, friend) {
	  return $http.get(server_host + '/users/' + user_id + '/subscribtions/'+ friend.user.id + '/edit.json').success(function(data){
...
function($http, search){
  ...
  o.getUsersPosts = function(id){
  	return $http.get( server_host + "/users/" + id + "/posts.json").success(function(res){
      angular.copy(res, o.posts);
    });
  };
  
  o.addPost = function(post){
  	return $http.post(server_host + "/users/" + post.user_id + '/posts.json', post).success(function(res){
      o.posts.push({post: res, root_comments: []});
    });
  };
  o.deletePost = function(post){
  	return $http.delete( server_host + "/users/" + post.post.post.user_id + '/posts/' + post.post.post.id + '.json').success(function(res){
...
function($http, posts, pictures, search, videos){
  ...
  o.addComment = function(commentable_id, commentable, comment, type, callback){
  	return $http.post(server_host + "/comments.json", {id: commentable_id, comment: comment, type: type}).success(function(res){
      if (type == "post"){
  ...
  o.deleteComment = function(commentable, comment, type){
  	return $http.delete( server_host + '/comments/' + comment.content.comment.id + '.json').success(function(res){
  		if (type == "post"){
			var post_index = posts.posts.indexOf(commentable);
			var comment_index;
			if(post_index != -1){
...
function($http, $sce){
  ...
  o.updateSong = function(song){
  	return $http.patch(server_host + '/songs/' + song.song.id + '.json', {title: song.song.title, performer: song.song.performer});
  };
...
function($http){
  ...
  o.getUsersPictures = function(id){
  	return $http.get(server_host + "/users/" + id + "/pictures.json").success(function(res){
  ...
  o.deletePicture = function(picture, callback){
  	...
  	return $http.delete(server_host + "/users/" + picture.picture.user_id + '/pictures/' + picture.picture.id + '.json').success(function(res){
      callback();
    });
  };
  ...
  o.setAvatar = function(picture){
  	return $http({
			    url: server_host + "/users/" + picture.user_id + ".json",
			    method: "PUT",
...
function($http, $sce, Notification){
  ...
  o.updateVideo = function(video){
  	return $http.patch( server_host + '/videos/' + video.video.video.id + '.json', {title: video.video.video.title});
  };
  
  o.copyVideo = function(id){
  	return $http.post( server_host + '/videos.json', {video: {id: id}}).success(function(res){
  ...
  o.deleteVideo = function(video){
  	return $http.delete( server_host + '/videos/' + video.video.video.id + '.json').success(function(res){
...
function($http){
  ...
  o.getUsersDialogues = function(id){
  	return $http.get( server_host + "/users/" + id + "/dialogues.json").success(function(res){
      angular.copy(res, o.dialogues);
    });
  };
  o.getUsersDialogue = function(id, dialogue_id){
  	return $http.get( server_host + "/users/" + id + "/dialogues/" + dialogue_id + ".json").success(function(res){
  ...
  o.sendMessage = function(user_id, dialogue_id, message){
  	return $http.post(server_host + "/users/" + user_id + "/messages.json", {dialogue_id: dialogue_id, message: message}).success(function(res){
        o.messages.push(res);
    });
  };
  o.sendMessagePage = function(message, sender_id, receiver_id){
  	return $http.post(server_host + "/users/" + sender_id + "/messages.json", {receiver_id: receiver_id, message: message}).success(function(res){

    });
  };
  o.notify = function(id, callback){
  	return $http.get( server_host + "/users/" + id + "/messages.json").success(function(res){
  		callback(res.new_messages);
    });
  };
  o.clearDialogueHistory = function(user_id){
  	return $http({
			    url: server_host + "/users/" + user_id + "/dialogues/" + o.dialogue_id + ".json",
			    method: "DELETE"
	 		});
  };
  o.deleteMessage = function(message){
  	return $http({
			    url: server_host + "/users/" + message.content.message.user_id + "/messages/" + message.content.message.id + ".json",
			    method: "DELETE"
	 		}).then(function(res){
...
function($http){
  ...
  o.addTag = function(tag, taggableType, taggableId) {
      return $http.post(server_host + '/tags.json', {type: taggableType, taggable_id: taggableId, tag: tag.text});
  };
...
function($http, posts, search, videos){
  ...
  o.like = function(likeable, type, callback){
  	...
  	var process;
  	if(type == "post"){
  		url = server_host + '/users/' + likeable.post.post.user_id + '/posts/' + likeable.post.post.id + '/toggle_like.json';
  		...
  		url = server_host + '/users/' + likeable.picture.picture.user_id + '/pictures/' + likeable.picture.picture.id + '/toggle_like.json';
  	}else if (type == "video"){
  		url = server_host + '/videos/' + likeable.video.video.id + '/toggle_like.json';
  		process = function(increment){
  		...
  		url = server_host + '/comments/' + likeable.comment.id + '/toggle_like.json';
  	}
  	if(callback){
  		process = callback;
  	}
...
function($http){
  ...
  o.getAll = function() {
      return $http.get(server_host + '/communities.json').success(function(res){
	      angular.copy(res, o.all);
	    });
  };
  
  o.get = function(id) {
      return $http.get(server_host + '/communities/' + id + '.json').success(function(res){
  ...
  o.getUsers = function(id){
  	return $http({
			    url: server_host + "/communities.json",
			    method: "GET",
			    params: {user_id: id}
	 		}).then(function(res){
  ...
  o.create = function(community, callback) {
      return $http.post(server_host + '/communities.json', community).success(function(res){
	      callback(res.id);
	    });
  };
  
  o.join = function(user) {
      return $http.get(server_host + '/communities/' + o.current.community.community.id + '/join.json').success(function(res){
  ...
  o.leave = function(user) {
      return $http.get(server_host + '/communities/' + o.current.community.community.id + '/leave.json').success(function(res){
